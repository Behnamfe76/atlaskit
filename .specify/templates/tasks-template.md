...
contracts, and adapter boundaries when those areas change. Adapter tasks MUST
also cover shared behavior contracts, reusable dynamic components, DOM mount
APIs where applicable, and styling customization paths.
...
- [ ] T006 [P] Establish shared package boundaries and public contract test harness
...
- [ ] T012 [P] [US1] Shared behavior contract test for adapter/core parity in [path]
...
- [ ] T019 [US1] If adapter work is in scope, implement or update
      `AtlasResourceTable`/`AtlasResourceForm`/`AtlasResourceShow`/
      `AtlasFieldRenderer`/`AtlasActionRunner` using smaller primitives,
      TailwindCSS utilities, and approved customization hooks only
...
- [ ] T022 [P] [US2] Shared behavior contract or styling customization test in [path]
...
- [ ] T027 [P] [US3] Contract test for [endpoint] in tests/contract/test\_[name].py
- [ ] T028 [P] [US3] Integration test for [user journey] in tests/integration/test\_[name].py
- [ ] T029 [P] [US3] Shared behavior contract or DOM mount API test in [path]
...
- [ ] TXXX [P] Additional unit tests required by changed behavior in tests/unit/
...
- For adapter features, include explicit tasks for behavior-contract parity,
  TailwindCSS class customization, and reusable component coverage
